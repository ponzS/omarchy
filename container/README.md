...
The image can be published by [the workflow](../.github/workflows/lightos-oci.yml) as `ghcr.io/ponzs/omarchy:lightos-core-4.0.4-r2`. The workflow packages the checked-out commit. The Omarchy package is pinned in pacman to retain the container specific dependency set; update this directory when publishing a later Omarchy release. Make the GHCR package public after its first publication so LightOS can pull it without registry credentials.

In LightOS, choose **Custom Image**, enter `ghcr.io/ponzs/omarchy:lightos-core-4.0.4-r2`, and create the instance with a non-root username and password. The image reference already contains its registry. On first boot, LightOS creates the requested user from Omarchy's `/etc/skel` and applies its normal instance setup. After the instance starts, use its terminal to inspect the Omarchy installation. When display takeover is enabled, the image starts a password-protected login on the VT allocated by LightOS. Logging in there starts the Omarchy desktop.
...
The desktop setup service grants the instance user access to the mapped GPU and starts seatd without switching VTs. It reads the actual VT from LightOS's `/dev/tty0` mapping so other instances' display terminals are left alone.
